...
public class ProcessDataWedgeOutputAsync extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(Void... voids) {
        ...
        String decodedMode = mData.getString(Constants.DataWedgeConstants.DATA_WEDGE_EXTRA_DECODE_MODE);

        // Handle Decode Modes
        switch(decodedMode) {
            case Constants.DataWedgeConstants.DATA_WEDGE_SINGLE_DECODE_MODE:
                handleSingleDecode();
                break;
            case Constants.DataWedgeConstants.DATA_WEDGE_MULTIPLE_DECODE_MODE:
    ...
    private void handleSingleDecode() {
        // Get Decode Data URI
        String decodeDataUri = mData.getString(Constants.DataWedgeConstants.DATA_WEDGE_EXTRA_DECODE_DATA);
        ...
        if(decodeDataUri != null) {
            ...
            if (cursor != null) {
                ...
                String id = cursor.getString(
                        cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SINGLE_BARCODE_ID));
                String labelType = cursor.getString(
                        cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SINGLE_BARCODE_LABEL_TYPE));
                String dataString = cursor.getString(
                        cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SINGLE_BARCODE_DATA_STRING));
                byte[] decodeData = cursor.getBlob(
                        cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SINGLE_BARCODE_DECODE_DATA));
                String nextDataUri = cursor.getString(
                        cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SINGLE_BARCODE_NEXT_DATA_URI));
                int fullDataSize = cursor.getInt(
                        cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SINGLE_BARCODE_FULL_DATA_SIZE));
                int dataBufferSize = cursor.getInt(
                        cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SINGLE_BARCODE_DATA_BUFFER_SIZE));
            ...
            handleEmptyCursor(Constants.DataWedgeConstants.DATA_WEDGE_SINGLE_DECODE_MODE);
        }
    }
    ...
    private void handleMultipleDecode() {
        ...
        ArrayList<Bundle> barcodesBundles = mData.getParcelableArrayList(Constants.DataWedgeConstants.DATA_WEDGE_EXTRA_DATA);

        // Loop Barcodes
        for (Bundle barcodeBundle : barcodesBundles) {
            // Grab URI from bundle
            String decodeDataUri = barcodeBundle.getString(Constants.DataWedgeConstants.DATA_WEDGE_EXTRA_FIELD_DATA_URI);
            ...
            if (decodeDataUri != null) {
                ...
                if (cursor != null) {
                    ...
                    String fieldId = cursor.getString(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_FIELD_ID));
                    String fieldType = cursor.getString(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_TYPE));
                    String fieldLabelType = cursor.getString(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_LABEL_TYPE));
                    String fieldStringData = cursor.getString(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_STRING_DATA));
                    byte[] fieldRawData = cursor.getBlob(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_RAW_DATA));
                    int fieldImageWidth = cursor.getInt(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_IMAGE_WIDTH));
                    int fieldImageHeight = cursor.getInt(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_IMAGE_HEIGHT));
                    int fieldSignatureStatus = cursor.getInt(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_SIGNATURE_STATUS));
                    int fullDataSize = cursor.getInt(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_FULL_DATA_SIZE));
                    int dataBufferSize = cursor.getInt(
                            cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_DATA_BUFFER_SIZE));

                    Field field;
                    if (fieldLabelType.equals(Constants.DataWedgeConstants.DATA_WEDGE_EXTRA_LABEL_TYPE_SIGNATURE)) {
                ...
                handleEmptyCursor(Constants.DataWedgeConstants.DATA_WEDGE_MULTIPLE_DECODE_MODE);
            }
        }
    ...
    private void handleEmptyCursor(String decodeMode) {
        // Extract Data
        String dataString = mData.getString(Constants.DataWedgeConstants.DATA_WEDGE_EXTRA_DATA_STRING);
        String labelType = mData.getString(Constants.DataWedgeConstants.DATA_WEDGE_EXTRA_LABEL_TYPE);
    ...
    private byte[] getBinaryDataFromCursor(Cursor cursor) throws Exception {
        ...
        // Get next URI
        String nextURI = cursor.getString(cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_NEXT_DATA_URI));

        // No data chunks - All data is available in one chunk
        if (nextURI.isEmpty()) {
            binaryData = cursor.getBlob(cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_RAW_DATA));
        } else {
            // Init OutputStream
            ByteArrayOutputStream baos = new ByteArrayOutputStream();

            // Get size of Data from Cursor
            final String fullDataSize = cursor.getString(cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_FULL_DATA_SIZE));

            // Create Buffer for raw data
            int bufferSize = cursor.getInt(cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_DATA_BUFFER_SIZE));

            // Write first chunk of data from cursor
            baos.write(cursor.getBlob(cursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_RAW_DATA)));
            ...
            while (!nextURI.isEmpty()) {
                ...
                if (imageDataCursor != null) {
                    ...
                    bufferSize += imageDataCursor.getInt(imageDataCursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_DATA_BUFFER_SIZE));
                    byte[] bufferData = imageDataCursor.getBlob(imageDataCursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_RAW_DATA));
                    baos.write(bufferData);
                    nextURI = imageDataCursor.getString(imageDataCursor.getColumnIndex(Constants.DataWedgeConstants.CURSOR_COLUMN_SIMULSCAN_NEXT_DATA_URI));
                }
